...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `requirements_get_untranslated` AS select `r`.`requirement_id` AS `requirement_id`,`r`.`project_id` AS `project_id`,`r`.`title` AS `title`,`r`.`description` AS `description`,`r`.`type` AS `type`,`r`.`rat` AS `rat`,`r`.`translated` AS `translated`,`r`.`processed` AS `processed`,`r`.`user_modified_at` AS `user_modified_at`,`r`.`bot_modified_at` AS `bot_modified_at`,`r`.`created_at` AS `created_at` from `requirements` `r` where (isnull(`r`.`translated`) or (`r`.`translated` = 0)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-01-28 20:29:54
